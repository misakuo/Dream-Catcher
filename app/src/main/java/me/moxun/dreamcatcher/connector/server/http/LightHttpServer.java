...
import me.moxun.dreamcatcher.connector.server.LeakyBufferedInputStream;
 ...
 * Performance is imperative here as Chrome aggressively polls DreamCatcher asking for
...
public class LightHttpServer {
    ...
    public void serve(SocketLike socket) throws IOException {
        ...
        while ((request = readRequestMessage(scratchRequest, reader)) != null) {
            ...
            LogUtil.w(LogUtil.filter(request, response));
    ...
    private boolean dispatchToHandler(
        ...
        if (handler == null) {
            ...
            response.body = LightHttpBody.create(build404Body(request.uri.getPath()), "text/html");
            return true;
        } else {
            if (handler instanceof WebSocketHandler) {
                LogUtil.e("dispatch to WS handler");
            }
            try {
    ...
    private String build404Body(String request) {
        String template = "<head>\n" +
                "<title>Handler Not Found</title>\n" +
                "</head>\n" +
                "<body>\n" +
                "<h1>DreamCatcher: handler not found</h1>\n" +
                "<p>Error code 404.\n" +
                "<p>Message: Handler not found.\n" +
                "<p>Error code explanation: 404 = No handler matches the given URI '%s'.\n" +
                "</body>";
